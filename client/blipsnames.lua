...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('kg-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('kg-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        KGCore.Functions.Notify(Lang:t('success.blips_activated'), 'success')
    else
        ShowBlips = false
        KGCore.Functions.Notify(Lang:t('error.blips_deactivated'), 'error')
    end
end)

RegisterNetEvent('kg-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        KGCore.Functions.Notify(Lang:t('success.names_activated'), 'success')
    else
        ShowNames = false
        KGCore.Functions.Notify(Lang:t('error.names_deactivated'), 'error')
    end
end)

RegisterNetEvent('kg-admin:client:Show', function(players)
