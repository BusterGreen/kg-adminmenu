...
RegisterNetEvent('kg-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('kg-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('kg-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('kg-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = KGCore.Functions.GetPlate(veh)
        local props = KGCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if KGCore.Shared.Vehicles[vehname] ~= nil and next(KGCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('kg-admin:server:SaveCar', props, KGCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            KGCore.Functions.Notify(Lang:t('error.no_store_vehicle_garage'), 'error')
        end
    else
        KGCore.Functions.Notify(Lang:t('error.no_vehicle'), 'error')
    end
end)
...
RegisterNetEvent('kg-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('kg-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == 'fast' then
...
RegisterNetEvent('kg-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('kg-admin:client:maxmodVehicle', function()
